...
const FixedPoint = __importStar(require('../math/lib/fixedPoint'));
...
function _exactTokensInForBPTOut(
    ...
    let weightedBalanceRatio = fixedPoint_1.fnum(0);
    ...
    for (let i = 0; i < balances.length; i++) {
        ...
        if (weightedBalanceRatio >= tokenBalanceRatiosWithoutFee[i]) {
            tokenBalancePercentageExcess = fixedPoint_1.fnum(0);
        } else {
