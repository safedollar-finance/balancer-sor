...
const weightedMath = __importStar(require('./pools/weightedPool/weightedMath'));
const elementMath = __importStar(require('./pools/elementPool/elementMath'));
const weightedSolidity = __importStar(
    require('./pools/weightedPool/weightedMathEvm')
);
...
exports.parsePoolPairData = (p, tokenIn, tokenOut) => {
    ...
    if (pairType != 'BPT->token') {
        ...
        if (poolType !== 'Stable')
            weightIn = bmath_1.bnum(tI.weight).div(bmath_1.bnum(p.totalWeight));
    }
    ...
    if (poolType == 'Weighted') {
        ...
        if (tokenIn == p.principalToken) {
            bnumBalanceIn = bnumBalanceIn.plus(bmath_1.bnum(p.lpShares));
        } else if (tokenOut == p.principalToken) {
            bnumBalanceOut = bnumBalanceOut.plus(bmath_1.bnum(p.lpShares));
        }
        poolPairData = {
            id: p.id,
            poolType: poolType,
            ...
            swapFee: bmath_1.bnum(p.swapFee),
            lpShares: bmath_1.bnum(p.lpShares),
            time: bmath_1.bnum(p.time),
        };
    } else {
        ...
        if (poolType == 'Weighted') {
            ...
            if (pairType == 'token->token') {
                returnAmount = weightedSolidity._exactTokenInForTokenOut(
                    ...
                    new FixedPointNumber_1.FixedPointNumber(
                        bmath_1.scale(weightIn, 18)
                    ),
                    new FixedPointNumber_1.FixedPointNumber(
                        bmath_1.scale(balanceOut, decimalsOut)
                    ),
                    new FixedPointNumber_1.FixedPointNumber(
                        bmath_1.scale(weightOut, 18)
                    ),
                    new FixedPointNumber_1.FixedPointNumber(
                        bmath_1.scale(amount, decimalsIn)
                    ),
                    new FixedPointNumber_1.FixedPointNumber(
                        bmath_1.scale(swapFee, 18)
                    )
                );
    ...
    for (let i = 0; i < hopTokens.length; i++) {
        ...
        for (let k in poolsTokenInNoTokenOut) {
            ...
            if (config_1.ALLOW_ADD_REMOVE)
                tokens.add(poolsTokenInNoTokenOut[k].id);
        ...
        for (let k in poolsTokenOutNoTokenIn) {
            ...
            // we can join/exit as part of the multihop
            if (config_1.ALLOW_ADD_REMOVE)
                tokens.add(poolsTokenOutNoTokenIn[k].id);
            if (tokens.has(hopTokens[i])) {
                let normalizedLiquidity = getNormalizedLiquidity(
                    exports.parsePoolPairData(
