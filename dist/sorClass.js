...
function getLimitAmountSwap(poolPairData, swapType) {
    ...
    if (swapType === types_1.SwapTypes.SwapExactIn) {
        return poolPairData.balanceIn.times(
            bmath_2.MAX_IN_RATIO.times(Math.pow(10, -18))
        );
    } else {
        return poolPairData.balanceOut.times(
            bmath_2.MAX_OUT_RATIO.times(Math.pow(10, -18))
        );
    }
}
        ...
        paths.forEach((path, j) => {
            ...
            if (path.limitAmount.gte(swapAmount)) {
                ...
                if (path.limitAmount.eq(swapAmount)) {
                    ...
                    effectivePrice = helpersClass_1.getEffectivePriceSwapForPath(
                        pools,
                        path,
                        swapType,
                        swapAmount
