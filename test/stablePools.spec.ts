...
import { fnum } from '../src/math/lib/fixedPoint';
import { _exactTokensInForBPTOut } from '../src/pools/stablePool/stableMathEvm';
...
describe(`Tests for Stable Pools.`, () => {
    ...
    it('debug tx 1', () => {
        const allBalances = [
            scale(bnum('20907.669071799866970388'), 18), // DAI
            scale(bnum('20952.250716'), 6), // USDC
            scale(bnum('20909.084189'), 6), // USDT
        ];
        const amp = bnum(200);
        const amounts = [
            scale(bnum('22915.785154407403193189'), 18), // DAI IN
            bnum(0),
            bnum(0),
        ];

        const bptTotalSupply = scale(bnum('62768.979200636283670271'), 18);
        const decimals = [18, 6, 6];

        const bptZeroPriceImpact = BPTForTokensZeroPriceImpact(
            allBalances,
            decimals,
            amounts, // This has to have the same lenght as allBalances
            bptTotalSupply,
        ...
        console.log(
            `${bptZeroPriceImpact.toString()} BPTForTokensZeroPriceImpact`
        );

        const fpBalances = [
            fnum(scale(bnum('20907.669071799866970388'), 18)), // DAI
            fnum(scale(bnum('20952.250716'), 18)), // USDC
            fnum(scale(bnum('20909.084189'), 18)), // USDT
        ];

        const fpAmounts = [
            fnum(scale(bnum('22915.785154407403193189'), 18)), // DAI IN
            fnum(0),
            fnum(0),
        ];

        const bptAmountEvm = _exactTokensInForBPTOut(
            fpBalances,
            fnum(amp),
            fpAmounts,
            fnum(bptTotalSupply),
            fnum(scale(bnum('0.0005'), 18))
        );
        console.log(`${bptAmountEvm.toString()} EVM Maths`);
        console.log(`22877844624660750468462 ACTUAL`);

        // Same method as UI
        const calcPriceImpact = bnum(1).minus(
            bptAmountEvm.div(bptZeroPriceImpact)
        );
        console.log(`${calcPriceImpact.toString()} calc price impact`);

        // 22915.785439959486954338 BPTForTokensZeroPriceImpact
        // 22877.844624660750468462 ACTUAL
        // 22877.844624660760000000 _exactTokensInForBPTOut
        expect(bptAmountEvm.toString()).eq('22877844624660750468462');
    });

    it('debug tx 2', () => {
        const allBalances = [
            scale(bnum('43823.454226207270163577'), 18), // DAI
            scale(bnum('20952.250716'), 6), // USDC
            scale(bnum('20909.084189'), 6), // USDT
        ];
        const amp = bnum(200);
        const amounts = [
            bnum(0),
            scale(bnum('20000'), 6), // USDC IN
            bnum(0),
        ];
        const bptTotalSupply = scale(bnum('85646.823825297034138733'), 18);
        const decimals = [18, 6, 6];
        ...
        const fpBalances = [
            fnum(scale(bnum('43823.454226207270163577'), 18)), // DAI
            fnum(scale(bnum('20952.250716'), 18)), // USDC
            fnum(scale(bnum('20909.084189'), 18)), // USDT
        ];
        ...
        const fpAmounts = [
            fnum(0),
            fnum(scale(bnum('20000'), 18)), // USDC IN
            fnum(0),
        ];
        ...
        console.log(`${bptAmountEvm.toString()} EVM Maths`);
        console.log(`19992074808307831171894 ACTUAL`);
        ...
        // 19996.029354188940425518 BPTForTokensZeroPriceImpact
        // 19992.074808307831171894 ACTUAL
        // 19992.074808307833000000 _exactTokensInForBPTOut
        expect(bptAmountEvm.toString()).eq('19992074808307831171894');
    });
    // });
});
